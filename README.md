...
从 `v3.0.10` 开始，`Yo` 对外提供纯粹的样式（`Style Only`）版本，即 `pure` 版本，策略及用法与之前的 `v1`, `v2` 版本相同。`pure` 版本与正常的版本进度保持一致，如需使用交互组件版本，请直接使用版本号相同的非 `pure` 版本。
...
* 安装: `bower install yo@3.0.10-pure`
